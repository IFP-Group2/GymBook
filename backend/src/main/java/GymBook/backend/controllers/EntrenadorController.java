...
public class EntrenadorController {
    ...
    public ResponseEntity<Entrenador> createEntrenador(@RequestBody Entrenador entrenador) {
        ...
        // Obtener el rol de entrenador
        Optional<Rol> rolOptional = rolService.findById(2L); // Suponiendo que el ID 2 es para entrenadores
        if (rolOptional.isEmpty()) {
            return ResponseEntity.badRequest().body(null); // Manejo de error si el rol no se encuentra
        }
        entrenador.setRol(rolOptional.get()); // Extraer el rol del Optional
